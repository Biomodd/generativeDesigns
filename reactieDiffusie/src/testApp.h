...
class testApp : public ofBaseApp{
	public:
        ...
        float dist(float x1, float y1, float x2, float y2); // to calculate the distance between two points
        void grabImage();

        float diffusion(float** chem,int x,int y); // calculate laplacian
		...
		bool mD; // mouse dragged
		int mX, mY; // mouseX, mouseY
